import React from "react";

interface IconProps {
  className?: string;
  width?: number | string;
  height?: number | string;
}

export const PyTorchIcon: React.FC<IconProps> = ({
  className = "",
  width = 24,
  height = 24,
}) => {
  return (
    <svg
      width={width}
      height={height}
      viewBox="0 0 424 512.501"
      xmlns="http://www.w3.org/2000/svg"
      shapeRendering="geometricPrecision"
      textRendering="geometricPrecision"
      imageRendering="optimizeQuality"
      fillRule="evenodd"
      clipRule="evenodd"
      className={className}
    >
      <g fill="#EE4C2C" fillRule="nonzero">
        <path d="M361.528 149.237l-38.178 38.179c62.472 62.472 62.472 163.12 0 224.435-62.472 62.472-163.12 62.472-224.435 0-62.472-62.472-62.472-163.122 0-224.435l98.914-98.915 13.882-13.882V0L62.472 149.237c-83.296 83.296-83.296 217.496 0 300.79 83.296 83.298 217.496 83.298 299.056 0 83.296-83.873 83.296-217.494 0-300.79z" />
        <path d="M314.675 112.218c0 15.334-12.431 27.765-27.766 27.765-15.336 0-27.767-12.431-27.767-27.765 0-15.335 12.431-27.766 27.767-27.766 15.335 0 27.766 12.431 27.766 27.766z" />
      </g>
    </svg>
  );
};

